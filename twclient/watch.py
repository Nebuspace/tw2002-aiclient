"""Settle-edge push-stream engine for `tw watch` / `tw spectate` (v2 A1).
...
class WatchHub:
    ...
    def broadcast_extra(self, event: dict):
        ...
        for events that aren't "the screen changed" at all (the Trainer
        Control Panel's AUTO-LOOP per-cycle progress, loop_player.py).
        Stamps `ts` the same way _maybe_emit()'s real settle-edge events
