"""twclient/trade_driver.py -- WO-FA4 the trade-CHAIN EXECUTION driver.
...
from .state_parser import credits_balance, parse_haggle, parse_state
...
def _current_strict_credits(session, caps, current_text: str) -> Optional[int]:
    ...
    on_screen = credits_balance(current_text)
    ...
    return bal if age_ms <= caps.credits_stale_ms else None
...
def _reconcile_turns(text: str, turns_budget: int) -> int:
    ...
    live = parse_state(text).get("turns_left")
    return live if live is not None else turns_budget


def _find_commodity_row(text: str, commodity: str) -> Optional[dict]:
    port = parse_state(text).get("port")
    if not port:
        return None
...
def _navigate(
    ...
    for next_sector in path[1:]:
        ...
        live_warps = parse_state(full_text).get("warps")
