...
class TerminalScreen:
    ...
    def color_map(self):
        """Per-cell color/bold attributes (D13), run-length-encoded per
        row for compactness, over the SAME bounding box as
        render_cropped(). Each row is a list of runs:
        {"start": c0, "end": c1 (exclusive), "fg": name, "bg": name,
        "bold": bool} — fg/bg are pyte's color names ("red", "green",
        "brown", "blue", "magenta", "cyan", "white", "black", "default").
        Sourced from pyte.Screen.buffer, which carries the SGR attributes
        the server actually sent (not just the plain text .display does).
        """
        max_row, max_col = self._cropped_bounds()
        if max_row == -1:
            return []
...
# per-glyph guess. Weight hierarchy (TUI-POLISH-PLAN.md Phase 1): double-
# line on the game VIEWPORT (heavier border = focus, BBS/DOS-door echo),
# thin rounded line on HUD chrome.
GLYPHS_UNICODE = {
    ...
    "delta_up": "▲",
    "delta_down": "▼",
    "bar_full": "█",   # Phase 4 dataviz: port %-meters, turns-left fuel gauge
    "bar_empty": "░",
    "sparkline": "▁▂▃▄▅▆▇█",  # Phase 4: credits sparkline ramp, low->high
}
GLYPHS_ASCII = {
    "viewport_tl": "+", "viewport_tr": "+",
    "viewport_bl": "+", "viewport_br": "+",
    ...
    "delta_up": "^",
    "delta_down": "v",
    "bar_full": "#",   # matches TUI-POLISH-PLAN.md's stated ASCII meter fallback "[###..]"
    "bar_empty": ".",
    "sparkline": ".-=#",
}
