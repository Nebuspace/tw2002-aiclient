...
* [test_chains.py](/testing/cases/test-chains.md) — TW-21 longest-profit-chain algorithm tests (synthetic graphs). 
* [test_classify.py](/testing/cases/test-classify.md) — Classify.
* [test_clean_preempt.py](/testing/cases/test-clean-preempt.md) — **REMOVED** (deleted from tip). Historical only. Was: Clean preempt. 
...
* [test_cli_crawl_wiring.py](/testing/cases/test-cli-crawl-wiring.md) — **REMOVED** (deleted from tip). Historical only. Was: Cli crawl wiring. 
* [test_cli_haggle_wiring.py](/testing/cases/test-cli-haggle-wiring.md) — **REMOVED** (deleted from tip). Historical only. Was: Cli haggle wiring. 
...
* [test_cli_players.py](/testing/cases/test-cli-players.md) — **REMOVED** (deleted from tip). Historical only. Was: `tw players` CLI verb tests -- no daemon involved, direct state/player_bank.json access (TW-31 client-side wiring, v1). 
...
* [test_control_panel.py](/testing/cases/test-control-panel.md) — **REMOVED** (deleted from tip). Historical only. Was: Control panel. 
* [test_crawl_start_protocol.py](/testing/cases/test-crawl-start-protocol.md) — Crawl start protocol. _(BANKED)_
* [test_credentials.py](/testing/cases/test-credentials.md) — Secure credential store tests (DESIGN-v2 B2) — no network, tmp_path only, never touches the real config/ directory. 
...
* [test_explore.py](/testing/cases/test-explore.md) — TW-14 Map-fill / frontier explore planner tests. 
* [test_fighter_toll_policy.py](/testing/cases/test-fighter-toll-policy.md) — WO-FIGHTER-FLOOR-TOLL — fighter reserve + Option? 
* [test_formations.py](/testing/cases/test-formations.md) — **REMOVED** (deleted from tip). Historical only. Was: TW-16 formation detector tests. 
* [test_frame_recorder.py](/testing/cases/test-frame-recorder.md) — **REMOVED** (deleted from tip). Historical only. Was: WO-FRAMES-0 — frame recorder + build_response hook + CLI read path. 
* [test_game_data.py](/testing/cases/test-game-data.md) — TW-24 game_data schema/loader tests. 
* [test_game_data_persist.py](/testing/cases/test-game-data-persist.md) — **REMOVED** (deleted from tip). Historical only. Was: Game data persist. 
* [test_game_knowledge.py](/testing/cases/test-game-knowledge.md) — **REMOVED** (deleted from tip). Historical only. Was: Game Knowledge Store tests (TW-25) -- no network, tmp_path only, never touches the real config/ or state/ directories. 
* [test_game_knowledge_learned_rules.py](/testing/cases/test-game-knowledge-learned-rules.md) — **REMOVED** (deleted from tip). Historical only. Was: Learned-rule store tests — offline, tmp_path only. 
...
* [test_haggle.py](/testing/cases/test-haggle.md) — Haggle. 
* [test_hud_seed.py](/testing/cases/test-hud-seed.md) — **REMOVED** (deleted from tip). Historical only. Was: WO-HUD-CREDITS-TURNS-JOIN — cold-join I-probe + sticky turns. 
* [test_iac.py](/testing/cases/test-iac.md) — IAC stripping + negotiation tests — no network involved.
* [test_integration_introspect_persist.py](/testing/cases/test-integration-introspect-persist.md) — **REMOVED** (deleted from tip). Historical only. Was: TW-26/27 introspector-to-persist chain integration test. 
* [test_interactive_app.py](/testing/cases/test-interactive-app.md) — **REMOVED** (deleted from tip). Historical only. Was: Interactive app. 
* [test_intervention_labels.py](/testing/cases/test-intervention-labels.md) — **REMOVED** (deleted from tip). Historical only. Was: Shared intervention label map — single source for play + spectate. 
* [test_introspector.py](/testing/cases/test-introspector.md) — TW-27 game-data introspector tests. 
* [test_ledger.py](/testing/cases/test-ledger.md) — Ledger. 
...
* [test_login_redaction.py](/testing/cases/test-login-redaction.md) — Login redaction. 
* [test_login_resume.py](/testing/cases/test-login-resume.md) — Login resume.
* [test_menu_crawler.py](/testing/cases/test-menu-crawler.md) — Menu Crawler tests (TW-26) -- no network, mock/fixture screens only. 
...
* [test_miner.py](/testing/cases/test-miner.md) — Miner. 
* [test_mode_badge_vocabulary.py](/testing/cases/test-mode-badge-vocabulary.md) — Mode badge vocabulary.
* [test_name_bank.py](/testing/cases/test-name-bank.md) — **REMOVED** (deleted from tip). Historical only. Was: Name bank tests (WO-MS-4 rider) -- no network, tmp_path only. 
...
* [test_probe.py](/testing/cases/test-probe.md) — **REMOVED** (deleted from tip). Historical only. Was: WO-MS-3 probe tests — classification, polite envelope, L0/L1 invariants. 
...
* [test_protocol_haggle.py](/testing/cases/test-protocol-haggle.md) — **REMOVED** (deleted from tip). Historical only. Was: Protocol haggle. 
* [test_protocol_trainer_panel.py](/testing/cases/test-protocol-trainer-panel.md) — **REMOVED** (deleted from tip). Historical only. Was: Protocol trainer panel. 
...
* [test_replay_ledger_integration.py](/testing/cases/test-replay-ledger-integration.md) — **REMOVED** (deleted from tip). Historical only. Was: Replay ledger integration. 
...
* [test_servers.py](/testing/cases/test-servers.md) — **REMOVED** (deleted from tip). Historical only. Was: WO-MS-1 server catalog tests. 
...
* [test_ship_upgrade_decision.py](/testing/cases/test-ship-upgrade-decision.md) — TW-30 ship-upgrade decision engine — unit coverage for all five §24 learnings. 
* [test_skills.py](/testing/cases/test-skills.md) — **REMOVED** (deleted from tip). Historical only. Was: Skill record/replay + playback tests (DESIGN-v2 §3 v2.1 item 11b/11d, C3) -- no network. 
...
* [test_state_parser.py](/testing/cases/test-state-parser.md) — **REMOVED** (deleted from tip). Historical only. Was: Best-effort state-extraction tests — no network involved. 
* [test_terminal.py](/testing/cases/test-terminal.md) — pyte render + crop correctness tests — no network involved.
* [test_trade_adapter.py](/testing/cases/test-trade-adapter.md) — Trade adapter. 
* [test_trade_driver.py](/testing/cases/test-trade-driver.md) — tests/test_trade_driver.py -- WO-FA4 trade_driver.py: the money-path chain-execution driver. 
...
* [test_world_identity.py](/testing/cases/test-world-identity.md) — World identity. 
* [test_world_model.py](/testing/cases/test-world-model.md) — World model tests (TW-06) -- no network, tmp_path only, never touches the real state/ directory. 
* [test_world_model_integration.py](/testing/cases/test-world-model-integration.md) — **REMOVED** (deleted from tip). Historical only. Was: World model integration. 