---
type: Reference
title: Test Cases — test_credentials
description: Secure credential store tests (DESIGN-v2 B2) — no network, tmp_path only, never touches the real config/ directory.
resource: repo://tw2002-aiclient/tests/test_credentials.py
tags: [testing, catalog, pytest, inventory]
timestamp: 2026-08-09T09:15:46Z
---

# Test Cases — `tests/test_credentials.py`

_Secure credential store tests (DESIGN-v2 B2) — no network, tmp_path only, never touches the real config/ directory._
...
| `test_missing_handle_raises_unless_allow_register_is_set` | Missing handle raises unless allow register is set. |
| `test_missing_handle_is_allowed_when_allow_register_is_true` | Missing handle is allowed when allow register is true. |
| `test_profile_explicit_flags_reflect_what_the_caller_actually_passed` | Profile explicit flags reflect what the caller actually passed. |
| `test_list_profiles` | List profiles. |
| `test_corrupt_profiles_toml_raises_credential_error_not_toml_decode` | WO-PROFILES-TOML-PARSE-HARDEN: tomllib failures must surface as recoverable CredentialError, never escape as TOMLDecodeError. |
| `test_load_profile_returns_last_good_profile_on_transient_parse_error` | Mid-write profiles.toml must not kill callers that already loaded this profile successfully -- return the cached last-good shape. |
...
| `test_save_password_never_writes_plaintext_to_profiles_toml` | The password must land ONLY in the secrets file, never anywhere near the (potentially-shared/less-locked-down) profiles.toml. |
