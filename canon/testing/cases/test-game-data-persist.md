---
type: Reference
title: Test Cases — Game Data Persist
description: TW-26/27 write lane -- persist/query path tests for `game_data.
resource: repo://tw2002-aiclient/tests/test_game_data_persist.py
tags: [testing, catalog, pytest, inventory]
timestamp: 2026-07-25T14:07:30Z
---

# Test Cases — `tests/test_game_data_persist.py` — **BANKED** (excluded from default test run; awaiting rehabilitation)
...
_TW-26/27 write lane -- persist/query path tests for `game_data.py`'s_
...
| `test_persist_ship_row_writes_into_the_existing_game_knowledge_store` | Not a second/parallel store -- the row must land inside the SAME. |
...
| `test_persist_ship_row_rejection_does_not_clobber_an_existing_good_row` | A later rejected write for the SAME ship must not touch the. |
...
| `test_cross_world_isolation_same_ship_name_different_values_per_world` | Two worlds may each introspect a ship of the same name with. |
...
| `test_real_concurrent_persist_ship_row_across_many_threads_never_loses_an_update` | N real threads, no monkeypatch synchronization, each persisting a. |
