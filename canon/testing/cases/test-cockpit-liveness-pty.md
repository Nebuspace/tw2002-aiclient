---
type: Reference
title: Test Cases — test_cockpit_liveness_pty
...
| `test_minimal_tier_cluster_present_despite_no_side_gutters` | Minimal tier cluster present despite no side gutters. |
...
| `test_poll_guard_fires_at_minimal_tier_because_of_control_strip` | Poll guard fires at minimal tier because of control strip. |
| `test_poll_guard_still_never_double_polls_when_control_strip_joins_other_consumers` | A tier where control_strip AND right_gutter/HUD are both present (the common case, e.g. |
...
| `test_raising_now_fn_does_not_crash_draw_and_cluster_still_renders` | Raising now fn does not crash draw and cluster still renders. |
