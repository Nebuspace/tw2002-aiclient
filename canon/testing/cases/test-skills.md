---
type: Reference
title: Test Cases — Skills
description: Skill record/replay + playback tests (DESIGN-v2 §3 v2.
resource: repo://tw2002-aiclient/tests/test_skills.py
tags: [testing, catalog, pytest, inventory]
timestamp: 2026-07-25T14:07:30Z
---

# Test Cases — `tests/test_skills.py` — **BANKED** (excluded from default test run; awaiting rehabilitation)
...
_Skill record/replay + playback tests (DESIGN-v2 §3 v2.1 item 11b/11d,_
...
| `test_record_then_replay_round_trip` | The full 11b loop, now including TW-03's start_anchor: SkillRecorder. |
...
| `test_play_skill_halts_on_floor_stop_loss` | WO-FA-SAFE Site C: the floor-check now reads the STRICT last-known. |
| `test_play_skill_halts_credits_unknown_when_no_balance_was_ever_observed` | Fail-closed on `None`: a session that never captured any balance. |
| `test_play_skill_halts_credits_unknown_on_a_session_without_the_supervision_surface` | Defensive hasattr-fallback: a session lacking `credits_snapshot()`. |
| `test_play_skill_halts_credits_unknown_on_a_stale_balance` | A real, KNOWN balance older than `credits_stale_ms` must be treated. |
| `test_play_skill_credits_stale_ms_is_config_driven_not_hardcoded` | The same stale reading `test_play_skill_halts_credits_unknown_on_a_. |
| `test_play_skill_proceeds_on_a_fresh_confirmed_balance_strictly_above_floor` | The proceed path: a fresh, confirmed balance strictly greater than. |
| `test_play_skill_price_mask_below_floor_poc_reads_the_strict_balance_not_the_loose_price_quote` | THE HUB'S MANDATORY PoC (Site C twin of loop_player. |
...
| `test_play_skill_halts_on_start_anchor_mismatch_mid_run` | The loop returns to a DIFFERENT sector than it started in --. |
...
| `test_replay_skill_stops_firing_further_sends_once_fenced_mid_run` | `is_driver_fenced` reads False on step 0's own check (nothing has. |
| `test_replay_skill_completes_every_step_when_never_fenced_sensitivity_control` | Sensitivity control for the test above: `is_driver_fenced` always. |
...
| `test_play_skill_halts_with_human_fenced_when_the_driver_is_fenced_mid_cycle` | `play_skill` reports a fence as its OWN distinct outcome, never. |
| `test_replay_skill_mid_animation_screen_change_halts_cleanly_not_a_misfire` | TW-02 (the -75-alignment/false-halt class, settle. |
| `test_replay_skill_slow_hub_warp_does_not_false_halt_on_premature_idle` | TW-02 companion: a warp-animation frame goes quiet just long. |
| `test_play_skill_treats_a_confirm_failure_as_a_surprise_halt` | play_skill wraps replay_skill in a generic `except. |
| `test_replay_skill_end_to_end_never_auto_fires_the_colonist_takeover_default` | DESIGN-v2 §8 -- the -75-alignment scar, proven END-TO-END through. |
