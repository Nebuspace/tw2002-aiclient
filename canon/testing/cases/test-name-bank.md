...
| `test_resolve_bank_identity_explicit_handle_always_wins` | Resolve bank identity explicit handle always wins. |
...
| `test_resolve_bank_identity_per_field_override_mixes_pinned_and_drawn` | A profile may pin ONE field while leaving the others to the bank -- per-field, not all-or-nothing. |
