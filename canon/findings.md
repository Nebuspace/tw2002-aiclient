...
| LOGIN-REDACTION-SUITE-NOT-RUNNING | `tests/test_login_redaction.py` still `import twclient` · pytest.ini `--ignore` | **DONE** tip `6278c1e` · `WO-AUDIT-LOGIN-REDACTION-REHAB` — suite rehabilitated onto `tw2002_aiclient`, un-ignored in `pytest.ini`; exception/return/ledger sinks pinned (22 collecting) |
| KEYS-ARGV-SHELL-HISTORY | `tw attach --keys` puts keystrokes on argv/process table/shell history; canon was silent; help/README lacked warn | **DONE** — doctrine Invariant 1 hazard + README caveat + ASCII `--keys` help (`NEVER a password - lands in argv/history`); residual argv surface is intentional for scripted attach, not a credential channel |
