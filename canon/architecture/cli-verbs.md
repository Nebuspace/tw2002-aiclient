...
| `players {list,next,rotate,add}` | **LIVE.** Multi-character rotation bank metadata (reads `state/player_bank.json`; no daemon, no game keystrokes). `list` prints bank rows + no-collusion boundary; `next` prints the next eligible profile; `rotate` prints the rotation driver's decision with reasoning; `rotate --check` is a passive notify-only inquiry (prints due/not-due with reason; exit 0 when the bank is readable even if nobody is due; exit 2 on `BankUnreadable`; never logs in / auto-switches); `add` appends a non-secret profile section via `credentials.create_profile()` (never logs in). | `--cooldown-hours H` on list/next/rotate; `--check` on rotate only; `add` takes `--server` / `--game-letter` / `--handle` / optional `--profile` | `read-only` (list/next/rotate) · `local-write` (add: profiles.toml only) | [Session Engine](/architecture/session-engine.md) · [Entry & Profile Selection](/surfaces/entry-and-profile-selection.md) |
...
| `crawl` | **TARGET (parked — Max-gated).** Human-invoked hub-supervised menu crawl against a `crawl_sacrificial` profile. Library `crawl_menus()` is tip-LIVE and test-proven; live driver / `tw crawl` / `crawl_start` rebuild waits on [PENDING-MENU-CRAWL-LIVE-DRIVER-REBUILD](/DECISIONS.md#PENDING-MENU-CRAWL-LIVE-DRIVER-REBUILD) · `WO-GATED-MENU-CRAWL-DRIVER-REBUILD`. Not orphan-dead — do not retire the never-commit suite. | `--profile` (required) `--max-nodes` `--path` | `drives {app}` | [Menu Map](/engine/menu-map-and-introspection.md) · [Session Engine](/architecture/session-engine.md) |
