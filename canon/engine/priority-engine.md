...
| 7 | Cost of fighters known | Boolean | 70 | #1 (Class-0 assumed reachable) | Partial — canon/`FIGHTER_UNIT_PRICE_CLASS0 = 100` remains an **UNVERIFIED hypothesis**. Tip has **no** constant by that name; explore posture uses a separately named placeholder `FIGHTER_UNIT_PRICE_DEFAULT = 100` (`session/explore_defensive_posture.py`) for routing math only. `afford_fighters` / GOALS still require injected `fighter_unit_price` / `fighter_price_class0` — **no** status producer writes a live Class-0 quote yet |
...
| 13 | Longest trade-loop chain identified | Range | 40 | ports known | Partial — `chains.find_profit_chains()`/`find_profit_chains_with_note()` + `rank_chains()` (re-verified 2026-08-08: prior citation `longest_profit_chain()` is dead code — zero product callers, docstring-only mentions; `rank_chains_by_yield()` added in PR #527 for `tw chains`/`cmd_chains` specifically); execute floor ≥2 links; ship hull deferred until ≥4 ([Chain execute-floor thresholds](#chain-execute-floor-thresholds-ordering-inputs) · `MIN_CHAIN_LINKS_FOR_SHIP_UPGRADE=4` — short chains fund fighters + holds first; see also [trade-loops](/strategy/trade-loops.md) § thresholds) |
...
- GOALS (`cockpit/goals.py`) paints `fighters_aboard`; when count is zero it uses an explicit
  ...
  (fail-closed `price?` until credits + unit price are known).
- **Buy EXECUTE** (one-shot Class-0 purchase mirroring `stardock_hold_driver` guard shape) remains
  **Planned** and **Max-gated** until (a) a live/captured Class-0 unit price feeds consumers and
  (b) Max GO's the money-path arm.
...
price; **do not** invent a tip constant that pretends the hypothesis is measured.
