...
### 1. `MODE_AI_PILOT` as a live-drive control-lock mode
...
Reborn north-star / control-and-escalation: live keyboard holders are **`{app, human}` only**; the
AI is a spectator-teacher that never live-drives. `MODE_AI_PILOT` as a *drive* mode has no canon
equivalent and must not return.

### 2. Per-cycle EV action-picker vs taught-behavior run-loop
...
### 3. Ledger actor enum `{ai, trainer, human}` vs `{app, human}`

**Canon:** `canon/architecture/session-engine.md` and `canon/engine/trace-ledger.md` (Code
Divergence sections).

Pre-rebirth `ledger.record_do()` used actor values **`{ai, trainer, human}`** (default `"ai"`),
treating LLM-decided sends as a live `ai` actor. Reborn send-time invariant: live senders are
**`{app, human}` only**; AI authorship is provenance of a *rule*, not a ledger live-actor value.
Do not migrate rows that stamp live sends as `ai`/`trainer`.

### 4. Verified 78-turn autopilot money-path misfire
