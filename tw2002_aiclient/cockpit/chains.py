...
``loops.store.read_loop_store`` found and answers which row is selected;
``app.py::_run_play`` owns the state transitions and the one call that
spends, and ``screens.py`` owns drawing. Same split ``teachband`` /
``autoloop_controls`` / ``panic`` already use.
...
chains the human can launch". **Taught** is the load-bearing word: this lists
macros a human already demonstrated, read from the shipped loop store. It
does **not** discover, rank, or score chains — no EV, no priority engine, no
chain finder (WO-PLAY-AUTOLOOP-START Scope 4). A row here is a thing the
operator taught, not a thing the app thinks is profitable.
...
def compose_chain_lines(
    ...
    *,
    ...
    The selected row carries canon's ``▸`` (ASCII ``>``); unselected rows are
    indented by the same width so the list does not shift horizontally as
    the cursor moves. An empty store renders canon's ``○ ○  no trade loop
    yet`` placeholder — a stated negative, never a blank box.
    """
    ...
    if not isinstance(rows, list) or not rows:
        ...
        if getattr(session, "status", "unreadable") == "ok":
            return [TITLE, f"{empty}  {EMPTY_TEXT}"]
        return [TITLE, f"{UNKNOWN}  {UNREADABLE_TEXT}"]
