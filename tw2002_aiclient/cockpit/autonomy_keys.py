...
``E)xplore`` · ``P/C/S`` App-armed policy toggles · ``T)rade Loop Chain`` ·
``L)ist Loops`` · Mode/^A App↔Manual (= halt). Hold?/Offer? remain wired
keys underneath but are **not** taught on this help surface (retired from
the calm band). App-armed + ·ON is the default automation path — no
confirm-gate doctrine here.
"""
...
EXPLORE_HELP = "E)xplore — App-armed infinite find-StarDock"
...
def compose_autonomy_help_lines() -> tuple[str, ...]:
    """Ordered calm-band key one-liners (E · P/C/S · Mode · L/T). Never raises."""
