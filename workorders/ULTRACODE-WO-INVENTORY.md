...
| PWO-025 | Control-lock + actor tag **LIVE** — lock+`VALID_SENDERS` + `LedgerWriter` + daemon do/send/attach wire (`protocol._record_ledger` / `record_attach_keystroke`) | harden | 020 | Every send tagged `{app,human}` | ledger rows actor∈{app,human} | ledger sample | control-and-escalation · trace-ledger |
...
> PREP: `workorders/WO-P6-080-088-autopilot-PREP.md` — tip honesty 2026-08-03: 080–086 LIVE; 087 LIVE (#337+#360 auto-haggle wire default OFF); 088 DONE/PARTIAL (kernel parked B). Do not invent Phase-5 chrome.
...
| PWO-087 | Auto-haggle as guarded rule — **LIVE** (`session/haggle.py` + tip tests #337; `trade_driver` opt-in `auto_haggle=False` default — WO-PWO-087 / Max GO). Canon caveat: [auto-haggle](../canon/engine/auto-haggle.md) § On-by-default distinguishes rule-macro ON-by-default from TradeChainRunner opt-in OFF. | harden | 081 | Built-in rule archetype | haggle tests | pytest+live | auto-haggle |
...
> PREP: `workorders/WO-P9-110-115-doctrine-PREP.md` — tip honesty 2026-08-04: 110/111(TX+RX-LOG)/112/113/114/115 LIVE; ledger via PWO-094 LIVE; 111 residuals = attach heuristic + live-paint echo.

| ID | Title | Type | Depends | Goal | Accept | Proof | Canon |
|----|-------|------|---------|------|--------|-------|-------|
| PWO-110 | Secrets resolve precedence — **LIVE** (`session/credentials.py`) | harden | 005 | env > secrets file | tests | secrets-and-credentials |
| PWO-111 | Redaction on send + RX transcript — **TX+RX-LOG LIVE** (`_log_tx`/`_log_rx`/`log_redacted` + LOGS; ledger via PWO-094; residuals: live-paint echo + attach heuristic non-keyword prompts) | harden | 020 | No password in logs/ledger | scan | secrets… |
