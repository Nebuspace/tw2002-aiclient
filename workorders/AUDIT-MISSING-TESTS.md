...
| MT-02 | **Login password never in return / `LoginError` / ledger — suite not collecting** — was `import twclient` + `--ignore`. | `login.py` invariant ("NEVER touches return values, exceptions") had no collecting test. | `tests/test_login_redaction.py` | **DONE** tip `6278c1e` · `WO-AUDIT-LOGIN-REDACTION-REHAB` | product + tests | Collecting suite green + un-ignored: sentinel absent from return / `LoginError` / ledger; falsification teeth kept. |
| MT-03 | **`tw status --json` stamps `daemon_running: True` over a failed round-trip** — SESSION-F7. | Exit code can be honest while JSON self-contradicts (PID reuse / dead sock). | `tests/test_cli_status_daemon_running_honesty.py` | **DONE** tip `b2ef693` · `WO-MT-03-STATUS-DAEMON-RUNNING` | product + tests | Pidfile-alive + failed RT → `daemon_running` false + `status_unreachable` + rc 1; success RT → `daemon_running` true. |
| MT-04 | **`tw watch` swallows unparseable frames** — SESSION-F8. | Corruption looks like a short stream; operators cannot tell frames were dropped. | `tests/test_cli_ops_verb_e2.py::test_unparseable_frame_*` | **DONE** tip `397f11d` · `WO-MT-04-WATCH-FRAME-SWALLOW` | product + tests | Corrupt line between valid events → stderr `ERROR: watch_frame_unparseable`; does not count toward `--frames N`. |
...
| MT-12 | **`cli.py` module docstring claims `tw watch` is the sole lifetime-stream exception** — SESSION-F1-MICRO-DOCSTRING; attach also holds a socket. No test (docs). | Operator/docs drift. | Docs tip (not pytest) | **DONE** tip `9110a95` · `WO-MT-12-WATCH-DOCSTRING` | docs-only | Tip module docstring names both `watch` and `attach` lifetime holds. |
