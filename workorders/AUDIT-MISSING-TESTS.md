...
| MT-09 | **Settle-nudge `send_request("read")` result discarded** — SESSION-F1-MICRO-SETTLE-NUDGE BANKED; no test that the discard stays benign or that a future check would surface errors. | Benign today; silent if nudge starts mattering. | `tests/test_cli_attach_settle_nudge.py` (optional micro) | **P2** | none / docs-only Accept ok | Pin: nudge failure does not flip attach rc; or product starts checking and test follows. |
...
| MT-11 | **`classify.py` breadth unaudited** — SESSION-AUDIT-COVERAGE-GAP; only secret-prompt regex heavily exercised. | False confidence that session audit closed classify. | Audit WO → then targeted tests | **P2** | future session/classify audit WO | Not a single test — track as audit dependency; do not invent mass tests this wave. |
