...
| Obtain fighters (aboard count > 0) | Boolean | 73 | credits known; Class 0 port found OR planet fighter source | **Partial** — GOALS **Fighters** line shows aboard count from HUD accumulator / `parse_state().fighters_aboard` / sticky `observe_fighters()`; `✓ N` when N>0, `· 0 (need some)` when zero. Class 0 port buy/deploy execute **Planned** (primary source per guide); StarDock Shipyards [P] also sells fighters at a premium as a convenience — not required. Depends on `fighter_toll_policy`: 0 fighters ⇒ Retreat on toll sectors. |
| Identification of cost of fighters | Boolean | 70 | Class 0 port found | **Planned** — fighter deploy/sell math exists (`fighter_toll_policy.py`) but Class 0 port fighter pricing is not a tracked goal yet. StarDock Shipyards [P] also offers fighters (same items, premium price) — Class 0 is the canonical price source. |
