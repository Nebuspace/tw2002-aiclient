...
| Identification of cost of other ships | Boolean | 80 | StarDock found | **Partial** — GOALS: `⊘ Ship prices need StarDock` until dock found; then `· price?` / `✓ N priced`. Catalog not yet on live `WorldSnapshot.ship_catalog`. |
| Identification of cost of cargo hold upgrades | Boolean | 75 | StarDock found | **Partial** — GOALS: `⊘ Hold upg need StarDock` until dock found; then `· price?` / `✓ N/h`. |
| Obtain fighters (aboard count > 0) | Boolean | 73 | **credits known** (Class-0 at Sol always reachable — location never the gate) | **Partial** — GOALS **Fighters** line shows aboard count from HUD accumulator / `parse_state().fighters_aboard` / sticky `observe_fighters()`; `✓ N` when N>0; when zero shows credit-gated status via `afford_fighters()`: `· 0 (can buy)` / `· 0 (need credits)` / `· 0 (holds first)` / `· 0 (price?)`. Class-0 at Sol (sector 1, game start) is **always reachable**; `⊘ need StarDock` must **never** gate this row. Buy execute **Planned** (navigation + port menu keystrokes). Depends on `fighter_toll_policy`: 0 fighters ⇒ Retreat on toll sectors. |
| Identification of cost of fighters | Boolean | 70 | credits known (Class-0 assumed reachable) | **Partial** — `FIGHTER_UNIT_PRICE_CLASS0 = 100 cr` is a documented placeholder (`priority_engine.py`); unverified against live game — confirm via introspected Class-0 port screen. `afford_fighters()` uses this constant as default when no observed price is available. StarDock Shipyards [P] also offers fighters at a premium — Class-0 is the canonical (cheaper) source. |
...
**Example:** trading a profit chain and considering a bigger ship at StarDock. Before abandoning the chain you need: (1) which ship and its price, (2) how far StarDock is (hop count on the known graph), (3) a **round-trip** turn budget — warp out, buy at dock, warp **back to the chain you were working** — which is roughly **2× the one-way warp cost** (plus dock/menu turns not modeled yet). Only then can you compare stay-trading vs leave-for-upgrade. Ship detours also wait until the best chain is ≥4 links (fighters/holds first at 3-link).
...
|---|---|---|---|
| GOALS | left PRIORITIES gutter (top) | Turns/credits, StarDock, map, formations, chain, ship prices, hold price, fighters | `spectate_app._build_goals_snapshot()` |
...
| FORMATIONS | left gutter under PRIORITIES | Discovered topologies by name + blurb | `compose_formations_panel()` |
