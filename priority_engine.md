...
| Obtain fighters (aboard count > 0) | Boolean | 73 | credits, StarDock or fighter source | **Partial** — GOALS **Fighters** line shows aboard count from HUD accumulator / `parse_state().fighters_aboard` / sticky `observe_fighters()`; `✓ N` when N>0, `· 0 (need some)` when zero. StarDock buy/deploy execute **Planned**. Depends on `fighter_toll_policy`: 0 fighters ⇒ Retreat on toll sectors. |
...
| Map 100% of Galaxy | Range (target 100%) | 45 | — | **Partial** — GOALS line is **Galaxy** progress (`N known (whole galaxy)` until total size known; then `N/total (pct%)`). Not a “map exactly N sectors” finish. `explore.known_graph()` + frontier BFS; galaxy size often unknown until fully mapped. |
